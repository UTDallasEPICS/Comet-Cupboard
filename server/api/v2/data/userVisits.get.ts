...
export default defineEventHandler(async (event) => {
	...
	const orders = await event.context.prisma.order.findMany({
		where: {
	...
	for (const { date, label, count } of rows) {
		const time = getTimeLevel(date, timeLevel)
	...
	// set visits per time level
	for (const time in allTimeLevels) {
		allTimeLevels[time] = allTimeLevels[time]["visits"] || 0
	}
