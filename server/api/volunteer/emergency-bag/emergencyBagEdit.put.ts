...
export default defineSafeHandler(async (event) => {
	const { bagID, bagCategory, expiryDate, items, oldItems, isPrivate, bagDescription } = await validateBody(event, schema)
	...
	return await prisma.$transaction(async (tx) => {
		...
		const originalMap = new Map(existingBag.EmergencyBagItems.map((i) => [i.itemID, i.count]))
		...
		for (const item of items) {
			mergedMap.set(item.itemID, (mergedMap.get(item.itemID) ?? 0) + item.count)
		}

		const finalItems = Array.from(mergedMap.entries()).map(([itemID, count]) => ({
			bagID,
			itemID,
			count,
		}))
