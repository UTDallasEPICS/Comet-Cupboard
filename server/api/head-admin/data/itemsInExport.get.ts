...
export default defineSafeHandler(async (event) => {
	...
	const sourceRows = itemChanges.map((row) => ({
	...
	const worksheet = workbook.addWorksheet("Items In")
	...
	if (format === "csv") {
		...
		setHeader(event, "Content-Disposition", 'attachment; filename="items-in.csv"')
		return buffer
	}

	const xlsxBuffer = await workbook.xlsx.writeBuffer()
	setHeader(event, "Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	setHeader(event, "Content-Disposition", 'attachment; filename="items-in.xlsx"')
	return xlsxBuffer
})
