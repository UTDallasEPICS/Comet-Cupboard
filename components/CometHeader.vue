    ...
    div.mx-4.sm_mx-8.flex.flex-row
        ...
        div.flex.flex-row.h-14.space-x-5.sm_space-x-14.items-center.w-full
            ...
            div.flex-grow.flex-row.space-x-10.overflow-x-scroll.no-scrollbar.items-center.left-left.text-3xl.font-bold.text-white.text-nowrap
                div(v-if="permissions['SHOPPING']").inline-flex.max-sm_hidden
                    NuxtLink(:to="shoppingPath").cursor-pointer.text-nowrap
                        | Shopping
                div(v-if="permissions['VERIFY_CART']").inline-flex.max-sm_hidden
...
div(v-show="mobileNav").sticky.top-20.z-40.flex-grow.h-screen.w-screen.bg-utd-green
    ...
    div.mt-10.flex.flex-col.space-y-7.text-white
        div(v-if="permissions['SHOPPING']").flex.justify-center.text-2xl.font-semibold
            NuxtLink(:to="shoppingPath", @click="mobileNav = false").cursor-pointer
                | Shopping
        div(v-if="permissions['VERIFY_CART']").flex.justify-center.text-2xl.font-semibold
...
const permissions = accessCookie.value && typeof accessCookie.value === "object" ? accessCookie.value : {}
