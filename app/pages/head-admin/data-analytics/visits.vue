...
const tableRows = computed(() => chartLabels.value.map((period) => ({ period, visits: totalVisits.value[period] ?? 0, uniqueVisitors: uniqueVisits.value[period] ?? 0 })))
...
const chartDatasets = computed(() => [
	{ label: "Total visits", data: tableRows.value.map((row) => row.visits), borderColor: "#e4701e", backgroundColor: "rgba(228,112,30,.12)", fill: true, tension: 0.3 },
	{ label: "Unique visits", data: tableRows.value.map((row) => row.uniqueVisitors), borderColor: "#154734", backgroundColor: "rgba(21,71,52,.08)", fill: true, tension: 0.3 },
])
const columns = [{ key: "period", label: "Period" }, { key: "visits", label: "Visits" }, { key: "uniqueVisitors", label: "Unique visits" }]
</script>