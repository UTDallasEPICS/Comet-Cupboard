...
const tableRows = computed(() =>
	Object.entries(inventory.value).flatMap(([category, items]) => Object.entries(items).map(([item, quantity]) => ({ category, item, quantity })))
)
const categoryTotals = computed(() =>
	Object.entries(inventory.value).map(([category, items]) => ({ category, quantity: Object.values(items).reduce((total, quantity) => total + quantity, 0) }))
)
