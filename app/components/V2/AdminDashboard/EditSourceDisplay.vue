...
div(class="min-w-screen rounded-2xl").bg-white.overflow-auto
    ...
    div(class="lg_w-[600px]").flex.flex-row.justify-center.mx-auto.h-7.lg_h-12.gap-3.w-60
        ...
        button(@click="addFieldToSource(sourceName)").bg-utd-green.font-semibold.font-montserrat.text-white.flex.items-center.justify-center.h-full.text-base.lg_text-3xl.rounded-xl.w-auto.p-2
            p + Add
            ...
            div(v-if="source.Fields?.length > 0" class="w-10/12 bg-[#eeeeee]").overflow-auto.ml-auto.mr-auto.rounded-xl.h-80.lg_h-96.mt-4
                ...
                div(v-for="field in source.Fields" class="w-11/12").flex.flex-row.rounded-xl.mx-auto.drop-shadow-standard.h-12.lg_h-20.bg-white.mt-2
                    ...
                    div.flex.items-center.justify-center.ml-auto.h-full.aspect-square
                        button(class="size-1/2" @click="removeField(field.fieldID)")
                            XMarkIcon
